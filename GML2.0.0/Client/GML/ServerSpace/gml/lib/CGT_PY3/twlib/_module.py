...
class _module:
    ...
    def get_id(a_fun, a_db, a_module, a_module_type, a_filter_list, a_limit="5000", a_start_num=""):
        ...
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(a_module_type,
                                                                                                      (str, str)) or \
                not isinstance(a_filter_list, list) or not isinstance(a_limit, (str, str)) or not isinstance(
            a_start_num, (str, str)):
            raise Exception("_module.get_id argv error,  must be( str,str,str,list, str, str)")

        # if not (type(a_db) == str and
        #         type(a_module) == str and
        #         type(a_module_type) == str and
        #         type(a_filter_list) == list and
        #         type(a_limit) == str and
        #         type(a_start_num) == str):
        #     raise Exception("_module.get_id argv error,  must be( str,str,str,list, str, str)")
        ...
        t_data = {"db": a_db, "module": a_module, "module_type": a_module_type, "sign_array": [t_id_sign],
                  "sign_filter_array": t_filter_list, "limit": a_limit, "order_sign_array": [],
    ...
    def get(a_fun, a_db, a_module, a_module_type, a_id_list, a_field_sign_list, a_limit="5000", a_order_sign_list=[]):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or \
                not isinstance(a_id_list, list) or not isinstance(a_field_sign_list, list) or not isinstance(a_limit, (
                str, str)) or not isinstance(a_order_sign_list, list):
        ...
        t_data_list = a_fun("c_orm", "get_in_id", {"db": a_db, "module": a_module, "module_type": a_module_type,
                                                   "sign_array": t_field_sign_list, "id_array": a_id_list,
                                                   "limit": a_limit, "order_sign_array": a_order_sign_list})
        t_result_list = []
    ...
    def set(a_fun, a_db, a_module, a_module_type, a_id_list, a_sign_data_dict):
        ...
        if len(a_id_list) == 0:
            return False
        return a_fun("c_orm", "set_in_id",
                     {"db": a_db, "module": a_module, "module_type": a_module_type, "id_array": a_id_list,
                      "sign_data_array": a_sign_data_dict})
    ...
    def get_dir(a_fun, a_db, a_module, a_module_type, a_id_list, a_folder_sign_list):
        ...
        if len(a_id_list) <= 0:
            return []
        t_os = _lib.get_os()
        t_data_list = a_fun("c_folder", "get_replace_path_in_sign",
                            {"db": a_db, "module": a_module, "module_type": a_module_type, "task_id_array": a_id_list,
                             "os": t_os, "sign_array": a_folder_sign_list})
        t_result_list = []
    ...
    def get_field_and_dir(a_fun, a_db, a_module, a_module_type, a_id_list, a_field_sign_list, a_folder_sign_list):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or \
                not isinstance(a_id_list, list) or not isinstance(a_field_sign_list, list) or not isinstance(
            a_folder_sign_list, list):
    ...
    @staticmethod
    def download_image(a_fun, a_http_ip, a_cgtw_path, a_token, a_db, a_module, a_module_type, a_id_list, a_field_sign,
                       a_is_small=True, a_local_path=""):
        if not isinstance(a_http_ip, (str, str)) or not isinstance(a_cgtw_path, (str, str)) or not isinstance(
                a_token, (str, str)) or \
                not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
            a_module_type, (str, str)) or \
                not isinstance(a_id_list, list) or not isinstance(a_field_sign, (str, str)) or not isinstance(
            a_is_small, bool) or not isinstance(a_local_path, (str, str)):
            raise Exception("_module.download_image argv error,  must be( str,str,str,list,str,bool,str)")

        t_os = _lib.get_os()

        t_ct_path = a_cgtw_path + "/ct"
        if t_ct_path not in sys.path:
            sys.path.append(t_ct_path)
        ...
        if a_is_small == True:
            t_size = "min"
        t_http = ct_http(a_http_ip, a_token)
        for data in t_data_list:
            t_download_list = []
    ...
    @staticmethod
    def set_image(a_fun, a_http_ip, a_cgtw_path, a_token, a_db, a_module, a_module_type, a_id_list, a_field_sign,
                  a_img_path, a_compress="1080"):
        if not isinstance(a_http_ip, (str, str)) or not isinstance(a_cgtw_path, (str, str)) or not isinstance(
                a_token, (str, str)) or \
                not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
            a_module_type, (str, str)) or \
        ...
        a_img_path_list = []
        if isinstance(a_img_path, (str, str)):
            if a_img_path.strip() == "":
                # 清空
        ...
        if not isinstance(a_img_path_list, (list)):
            raise Exception("_module.set_image argv error , a_img_path type in (list/str/str)")
        ...
        return _module.set(a_fun, a_db, a_module, a_module_type, a_id_list,
                           {a_field_sign: _lib.encode({"max": t_big_list, "min": t_small_list})})
    ...
    def count(a_fun, a_db, a_module, a_module_type, a_filter_list):
        ...
        t_filter_list = _module.__change_filter(a_module, a_module_type, a_filter_list)
        return a_fun("c_orm", "get_count_with_filter",
                     {"db": a_db, "module": a_module, "module_type": a_module_type, "sign_filter_array": t_filter_list})

    @staticmethod
    def distinct(a_fun, a_db, a_module, a_module_type, a_filter_list, a_field_sign, a_order_sign_list=[]):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or \
                not isinstance(a_filter_list, list) or not isinstance(a_field_sign, (str, str)) or not isinstance(
            a_order_sign_list, list):
            raise Exception("_module.distinct argv error,  must be( str,str,str,list, str,list)")
        t_filter_list = _module.__change_filter(a_module, a_module_type, a_filter_list)
        t_data_list = a_fun("c_orm", "get_distinct_with_filter",
                            {"db": a_db, "module": a_module, "module_type": a_module_type,
                             "distinct_sign": a_field_sign, "sign_filter_array": t_filter_list,
                             "order_sign_array": a_order_sign_list})
        t_result_list = []
    ...
    def create(a_fun, a_db, a_module, a_module_type, a_sign_data_dict, a_is_return_id=False):
        ...
        if str(a_db).lower().strip() == "public" and str(a_module).lower().strip() in ["account", "project"]:
        ...
        if not a_is_return_id:
            return a_fun("c_orm", "create", {"db": a_db, "module": a_module, "module_type": a_module_type,
                                             "sign_data_array": a_sign_data_dict})
        else:
            t_id = _lib.uuid()
            a_sign_data_dict[a_module + ".id"] = t_id
            res = a_fun("c_orm", "create", {"db": a_db, "module": a_module, "module_type": a_module_type,
                                            "sign_data_array": a_sign_data_dict})
            if res == True:
                return t_id
            else:
    ...
    def get_makedirs(a_fun, a_db, a_module, a_module_type, a_id_list):
        ...
        for i in range(t_num):
            ...
            res = a_fun("c_folder", "get_create_folder_data",
                        {"db": a_db, "module": a_module, "module_type": a_module_type, "os": t_os,
                         "task_id_array": temp_id_lis})
    ...
    def create_task(a_fun, a_db, a_module, a_module_type, a_join_id, a_pipeline_id, a_task_name, a_flow_id):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or \
                not isinstance(a_join_id, (str, str)) or not isinstance(a_pipeline_id,
                                                                        (str, str)) or not isinstance(
            a_task_name, (str, str)) or not isinstance(a_flow_id, (str, str)):
            raise Exception("_module.create_task argv error,  must be( str,str,str,str,str,str,str)")
        if str(a_join_id).strip() == "" or str(a_pipeline_id).strip() == "" or str(
                a_task_name).strip() == "":
            raise Exception("_module.create_task argv error,  can not empty( a_join_id, a_pipeline_id, a_task_name)")

        t_post_data = {"db": a_db, "module": a_module, "module_type": a_module_type, "join_id": a_join_id,
                       "pipeline_id": a_pipeline_id, "flow_id": a_flow_id, "task_name": a_task_name};
    ...
    def assign(a_fun, a_db, a_module, a_module_type, a_id_list, a_assign_account_id, a_start_date="", a_end_date=""):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or \
                not isinstance(a_id_list, list) or not isinstance(a_assign_account_id,
                                                                  (str, str)) or not isinstance(a_start_date, (
                str, str)) or not isinstance(a_end_date, (str, str)):
            raise Exception("_module.assign argv error,  must be( str,str,str,list,str,str,str)")
        if len(a_id_list) == 0:
            return False
        return a_fun("c_work_flow", "assign_to", {'db': a_db, 'module': a_module, 'module_type': a_module_type,
                                                  'assign_account_id': a_assign_account_id, 'start_date': a_start_date,
                                                  'end_date': a_end_date, 'task_id_array': a_id_list})

    @staticmethod
    def submit(a_fun, a_account_id, a_db, a_module, a_module_type, a_id, a_file_path_list, a_note="", a_path_list=[]):
        if not isinstance(a_account_id, (str, str)) or not isinstance(a_db, (str, str)) or not isinstance(
                a_module, (str, str)) or not isinstance(a_module_type, (str, str)) or \
                not isinstance(a_id, (str, str)) or not isinstance(a_file_path_list, list) or not isinstance(a_note,
                                                                                                             (str,
        ...
        t_dic = {"#link_id": a_id, "version": "", "filename": basename_list, "local_path": a_path_list, "web_path": [],
                 "sign": "Api Submit", "image": "", "from_version": "", "is_upload_web": "N", "#id": version_id}
        res = a_fun("c_version", "create", {"db": a_db, "field_data_array": t_dic})
        if res != True:
            return False
        return a_fun("c_work_flow", "submit",
                     {"db": a_db, "module": a_module, "module_type": a_module_type, "task_id": a_id,
                      "account_id": a_account_id, "submit_file_path_array": t_submit_file_path_array, "text": t_note,
                      "version_id": version_id})

    @staticmethod
    def update_flow(a_fun, a_http_ip, a_cgtw_path, a_token, a_db, a_module, a_module_type, a_id, a_field_sign, a_status,
                    a_note="", a_image_list=[]):
        if not isinstance(a_http_ip, (str, str)) or not isinstance(a_cgtw_path, (str, str)) or not isinstance(
                a_token, (str, str)) or not isinstance(a_db, (str, str)) or not isinstance(a_module, (
                str, str)) or not isinstance(a_module_type, (str, str)) or \
                not isinstance(a_id, (str, str)) or not isinstance(a_field_sign, (str, str)) or not isinstance(
            a_status, (str, str)) or not isinstance(a_note, (str, str)) or not isinstance(a_image_list, list):
        ...
        t_note_data = ""
        if a_note.strip() != "":
            t_note_data = json.dumps({"data": a_note, "image": image_list})
        return a_fun("c_work_flow", "python_update_flow",
                     {"db": a_db, "module": a_module, "module_type": a_module_type, "field_sign": a_field_sign,
                      "text": t_note_data, "status": a_status, "task_id": a_id})

    @staticmethod
    def get_submit_filebox(a_fun, a_db, a_module, a_module_type, a_id):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or \
                not isinstance(a_id, (str, str)):
            raise Exception("_module.get_submit_filebox argv error,  must be( str,str,str,str)")
        if unicode(a_id).strip() == "":
            return []
        t_os = _lib.get_os()
        return a_fun("c_file", "filebox_get_submit_data",
                     {"db": a_db, "module": a_module, "module_type": a_module_type, "task_id": a_id, "os": t_os})

    @staticmethod
    def get_sign_filebox(a_fun, a_db, a_module, a_module_type, a_id, a_filebox_sign):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or \
                not isinstance(a_id, (str, str)) or not isinstance(a_filebox_sign, (str, str)):
        ...
        if unicode(a_id).strip() == "":
            return []
        t_os = _lib.get_os()
        return a_fun("c_file", "filebox_get_one_with_sign",
                     {"db": a_db, "module": a_module, "module_type": a_module_type, "task_id": a_id, "os": t_os,
                      "sign": a_filebox_sign})
    ...
    def get_filebox(a_fun, a_db, a_module, a_module_type, a_id, a_filebox_id):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or \
                not isinstance(a_id, (str, str)) or not isinstance(a_filebox_id, (str, str)):
        ...
        if unicode(a_id).strip() == "":
            return []
        t_os = _lib.get_os()
        return a_fun("c_file", "filebox_get_one_with_id",
                     {"db": a_db, "module": a_module, "module_type": a_module_type, "task_id": a_id, "os": t_os,
                      "filebox_id": a_filebox_id})

    @staticmethod
    def modules(a_fun, a_db, a_module_type):
        t_module_list = []
        t_field_list = ["module", "is_link_task"]
        t_data_list = a_fun("c_module", "get_with_filter",
                            {"db": a_db, "field_array": t_field_list, "filter_array": [["type", "=", "info"]]})
    ...
    @staticmethod
    def fields(a_fun, a_db, a_module, a_module_type):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)):
            raise Exception("_module.__fields argv error,  must be( str, str)")
        data_list = _module.__fields(a_fun, a_db, a_module, a_module_type, ["sign"])
        t_lis = []
    ...
    @staticmethod
    def fields_and_str(a_fun, a_db, a_module, a_module_type):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)):
            raise Exception("_module.__fields argv error,  must be( str, str)")
        data_list = _module.__fields(a_fun, a_db, a_module, a_module_type, ["sign", "field_str"])
        t_lis = []
        if isinstance(data_list, list):
            for i in data_list:
                if isinstance(i, list) and len(i) > 1:
                    t_lis.append({"sign": i[0], "field_str": i[1]})
        return t_lis

    @staticmethod
    def group_count(a_fun, a_db, a_module, a_module_type, a_field_sign_list, a_filter_list):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or not isinstance(a_field_sign_list, list) or not isinstance(
            a_filter_list, list):
            raise Exception("_module.group_count argv error,  must be( str, str, str, list, list)")

        t_filter_list = _module.__change_filter(a_module, a_module_type, a_filter_list)
        t_data_list = a_fun("c_orm", "get_group_count_with_filter",
                            {"db": a_db, "module": a_module, "module_type": a_module_type,
                             "sign_array": a_field_sign_list, "sign_filter_array": t_filter_list})
        t_result_list = []
        for t_data in t_data_list:
            t_tmp_dict = {}
            if len(t_data) - 1 == len(a_field_sign_list):
                for i in range(len(t_data)):
                    if i == 0:
                        t_tmp_dict["count"] = t_data[0]
                    else:
                        t_tmp_dict[a_field_sign_list[i - 1]] = t_data[i]
            t_result_list.append(t_tmp_dict)
        return t_result_list

    # ------------------------------------------------------------------------
    @staticmethod
    def __fields(a_fun, a_db, a_module, a_module_type, a_field_array):
        if not isinstance(a_db, (str, str)) or not isinstance(a_module, (str, str)) or not isinstance(
                a_module_type, (str, str)) or not isinstance(a_field_array, list):
            raise Exception("_module.__fields argv error,  must be( str, str, str, list)")
        return a_fun("c_field", "get_join_module_data",
                     {"db": a_db, "module": a_module, "module_type": a_module_type, "field_array": a_field_array,
    ...
    def __change_asset_data(a_fun, a_db, a_sign_data_dict):
        ...
        if not a_sign_data_dict.has_key("asset.asset_name") and not a_sign_data_dict.has_key("asset.type_name"):
            raise Exception(
                "_module.__change_asset_data, a_sign_data_dict must have key (asset.asset_name,  asset.type_name)")
        t_sign_data_dict = a_sign_data_dict.copy()
        t_type_id = a_fun("c_config", "get_one_with_filter", {"table": a_db + ".conf_type", "field": "#id",
                                                              "filter_array": [["entity_name", "=",
                                                                                t_sign_data_dict["asset.type_name"]]]})
    ...
    def __change_shot_data(a_fun, a_db, a_sign_data_dict):
        ...
        替换shot的创建数据
        '''
        if "eps.eps_name" not in a_sign_data_dict.keys() and "eps.id" not in a_sign_data_dict.keys() and \
                "shot.shot" not in a_sign_data_dict.keys():
            raise Exception(
                "_module.__change_shot_data, a_sign_data_dict must have key (eps.eps_name/eps.id,  shot.shot)")
        t_sign_data_dict = a_sign_data_dict.copy()
        print(t_sign_data_dict)
        if "eps.eps_name" in t_sign_data_dict.keys():
            print(a_fun)
            t_eps_id = a_fun("c_orm", "get_one_with_filter",
                             {"db": a_db, "module": "eps", "module_type": "info", "sign": "eps.id",
                              "sign_filter_array": [["eps.eps_name", "=", t_sign_data_dict["eps.eps_name"]]]})
        ...
        elif "eps.id" in t_sign_data_dict.keys():
