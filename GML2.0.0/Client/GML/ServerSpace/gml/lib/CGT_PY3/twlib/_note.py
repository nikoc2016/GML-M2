...
class _note:
    ...
    @staticmethod
    def create(a_fun, a_http_ip, a_cgtw_path, a_token, a_account_id, a_db, a_module, a_module_type, a_task_id_list, a_text, a_cc_acount_id="", a_image_list=[],is_return_id=False):
        if not isinstance(a_http_ip, (str, unicode)) or not isinstance(a_cgtw_path, (str, unicode)) or not isinstance(a_account_id, (str, unicode)) or not isinstance(a_token, (str, unicode)) or \
           ...
           not isinstance(a_task_id_list, list) or not isinstance(a_text, (str, unicode)) or not isinstance(a_cc_acount_id, (str, unicode)) or not isinstance(a_image_list, list) or not isinstance(is_return_id,bool):
            raise Exception("_note.create argv error,  must be( str,str,str,str,str,str,str,list,str,str,list,bool)")
        ...
        t_sign_data_dict = {"db":a_db, "cc_acount_id":a_cc_acount_id, "field_data_array":{"module":a_module,  "module_type":a_module_type, "#task_id":",".join(a_task_id_list), "text":{"data":a_text, "image":image_list}, "#from_account_id":a_account_id }}
        if not is_return_id:
            return a_fun("c_note", "create", t_sign_data_dict)
        else:
            t_id = _lib.uuid()
            t_sign_data_dict['field_data_array']['#id'] = t_id
            res=a_fun("c_note", "create", t_sign_data_dict)
            if res==True:
                return t_id
            else:
                return False            
