...
class _pipeline:
    ...
    def get_id(a_fun, a_db, a_filter_list):
        if not isinstance(a_db, str) or not isinstance(a_filter_list, list):
    ...
    def get(a_fun, a_db, a_id_list, a_field_list):
        if not isinstance(a_db, str) or not isinstance(a_id_list, list) or not isinstance(a_field_list, list):
    ...
    def fields():
        ...
        # "module_type": u"模块类型",
        # }
        return ["#id", "entity_name", "module", "module_type", "description"]
